...
library utils {
    ...
    function random(uint input, uint min, uint max) public pure returns (uint) {
...
library segments {
    ...
    function getNumbers(
        uint input,
        uint length
    ) internal pure returns (Number memory result) {
        if (length == 1) {
            result.d = input;
        } else if (length == 2) {
            result.c = (input / 10);
...
contract Uints is ERC721A, Ownable, IERC4906 {
    ...
    function tokenURI(
        ...
        string memory svg = segments.renderSvg(value, getRgbs(tokenId));
